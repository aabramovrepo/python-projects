...
plane_alpha = [.3, .25, .2, .15, .1]   # transparency level for drawing hyperplanes
...
def generate_point_list_3d(n, min_val, max_val):
    ...
    for i in range(n):
        ...
        #p.append((random.randint(min_val,max_val), random.randint(min_val,max_val), random.randint(min_val,max_val)))
...
def draw_sphere(ax, origin, radius, surface_color, surface_alpha):
    ...
    ax.plot_surface(x_3d, y_3d, z_3d, cstride=1, rstride=1, color=surface_color, linewidth=0., alpha=surface_alpha)


def plot_tree_3d(tree, ax, min_x, max_x, min_y, max_y, min_z, max_z, prev_node, branch, vis_planes=True, depth=0):
    """ draw 3D K-D tree
    ...
    if axis == 0:
        ...
        if vis_planes:
            ...
            ax.plot_surface(cur_node[0], yz_y, yz_z, cstride=1, rstride=1, color='#00cc00', linewidth=0.05, alpha=pl_alpha)
    ...
    # draw y splitting plane
        ...
        if vis_planes:
            ...
            ax.plot_surface(xz_x, cur_node[1], xz_z, cstride=1, rstride=1, color='#0066ff', linewidth=0.05, alpha=pl_alpha)
    ...
    # draw z splitting plane
        ...
        if vis_planes:
            ...
            ax.plot_surface(xy_x, xy_y, cur_node[2], cstride=1, rstride=1, color='#ff1a00', linewidth=0.05, alpha=pl_alpha)
    ...
    if left_branch is not None:
        plot_tree_3d(left_branch, ax, min_x, max_x, min_y, max_y, min_z, max_z, cur_node, True, vis_planes, depth+1)

    if right_branch is not None:
        plot_tree_3d(right_branch, ax, min_x, max_x, min_y, max_y, min_z, max_z, cur_node, False, vis_planes, depth+1)
...
def nearest_neighbor_search(tree, target_point, hr, distance, nearest=None, depth=0):
    ...
    dist = (cur_node[0] - target_point[0])**2 + (cur_node[1] - target_point[1])**2 + (cur_node[2] - target_point[2])**2
    ...
    py = compute_closest_coordinate(target_point[1], further_hr[0][1], further_hr[1][1])
    pz = compute_closest_coordinate(target_point[2], further_hr[1][2], further_hr[0][2])

    # check whether it is closer than the current nearest neighbor => whether the hypersphere crosses the hypercube
    dist = (px - target_point[0])**2 + (py - target_point[1])**2 + (pz - target_point[2])**2
...
def kd_tree_nn_search(step):
    ...
    point = (random.randint(min_val,max_val), random.randint(min_val,max_val), random.randint(min_val,max_val))

    delta = 2  # extension of the drawing range
    hr = [(min_val-delta, min_val-delta, max_val+delta), (max_val+delta, max_val+delta, min_val-delta)]  # initial hyper cube
    ...
    if visual:
        ...
        vis_planes = True  # True if hyperplanes need to be plotted, False otherwise

        plot_tree_3d(kd_tree, ax, min_val-delta, max_val+delta, min_val-delta, max_val+delta, min_val-delta, max_val+delta, None, None, vis_planes)
    ...
    for ind in range(len(point_list)):
        ...
        dist = math.sqrt( (point_list[ind][0] - point[0])**2 + (point_list[ind][1] - point[1])**2 + (point_list[ind][2] - point[2])**2 )
        dist_2 = (point_list[ind][0] - point[0])**2 + (point_list[ind][1] - point[1])**2 + (point_list[ind][2] - point[2])**2
    ...
    if nearest_nn[0] != nn_point[0] or nearest_nn[1] != nn_point[1] or nearest_nn[2] != nn_point[2]:
        ...
        if min_distance_2 < distance_nn:
            ...
            print 'point = ', point
            print 'min_distance_2 = ', min_distance_2
